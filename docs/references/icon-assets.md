...
- **目录**：`src/SysPulseExtension/SysPulseExtension/Assets/`
...
- **目录**：`installer/`（放 `SysPulse.ico`，与 `SysPulse.iss` 同级）。
...
- **卸载显示图标**：`UninstallDisplayIcon={app}\Host\SysPulse.Host.exe` —— 取 **Host exe 内嵌图标**。
...
- **目录**：`src/SysPulse.Host/`（放 `SysPulse.Host.ico`，或复用 §2 的同一 `.ico`）。
- **csproj 加**（`SysPulse.Host.csproj`）：
  ```xml
  <ApplicationIcon>SysPulse.Host.ico</ApplicationIcon>
  ```
...
- [ ] 多尺寸 `.ico` ×1（16–256），放 `installer/` 与 `src/SysPulse.Host/`（或一份两处引用）
