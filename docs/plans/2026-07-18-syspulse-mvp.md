# SysPulse MVP Implementation Plan
...
**Architecture:** 双进程 —— ① `SysPulse.Host`：提权后台进程，用 LibreHardwareMonitorLib 读传感器，经命名管道以 JSON 快照对外提供；② `SysPulseExtension`：CmdPal MSIX 扩展，通过 `ICommandProvider3.GetDockBands()` 提供 Dock band，定时轮询管道并更新 `ListItem` 的 Title/Subtitle 实现实时刷新。扩展检测到 Host 未运行时自动（UAC）拉起。设计依据见 `docs/architecture.md`。
...
├─ docs/                          # 已存在（本规划与参考文档）
├─ src/
│  ├─ SysPulseExtension/     # CmdPal 模板生成（Task 1），MSIX 打包
│  │   SysPulseExtension.csproj
│  │   SysPulseExtensionCommandsProvider.cs
...
│  └─ SysPulse.Host/         # Task 2 起
│      SysPulse.Host.csproj
...
**Files:**
- Create: `src/SysPulseExtension/`（由 CmdPal 模板生成器产出整个项目）
...
- ExtensionName: `SysPulseExtension`
- Extension Display Name: `SysPulse`
- Output Path: `D:\Workspace\SysPulse\src`（生成器会在其下建同名子目录）
...
打开 `src/SysPulseExtension` 下的 `Directory.Packages.props`（或 csproj），确认 `Microsoft.CommandPalette.Extensions` ≥ `0.9.260303001`；不足则升级该 PackageReference。

- [ ] **Step 5: 部署并验证基础扩展**

Visual Studio 打开 `SysPulseExtension.sln` → 菜单 `Build` → `Deploy SysPulseExtension`（注意必须 **Deploy**，仅 Build 不更新包）。然后在命令面板运行 `Reload`（副标题为 Reload Command Palette Extension），列表底部应出现 `SysPulse` 占位命令。

- [ ] **Step 6: 加一个最小 Dock band 验证 Dock 链路**

在 `SysPulseExtensionCommandsProvider.cs` 中覆写：

```csharp
public override ICommandItem[]? GetDockBands()
{
    var page = new SysPulseExtensionPage(); // 模板自带的页
    return [new CommandItem(page) { Title = "SysPulse" }];
}
```

重新 Deploy + Reload → PowerToys 设置里把本扩展的 band 添加到 Dock → 屏幕边缘 Dock 上应出现 `SysPulse` 按钮。
...
## Phase 1 — SysPulse.Host（传感器数据源）

### Task 2: Host 项目骨架 + SensorSnapshot 模型

**Files:**
- Create: `SysPulse.sln`（把 Host/Tests/Extension 收进同一 sln）
- Create: `src/SysPulse.Host/SysPulse.Host.csproj`
- Create: `src/SysPulse.Host/Model/SensorSnapshot.cs`
- Test: `tests/SysPulse.Host.Tests/SensorSnapshotTests.cs`

- [ ] **Step 1: 建项目与解决方案**

```bash
cd /d/Workspace/SysPulse
dotnet new sln -n SysPulse
dotnet new console -n SysPulse.Host -o src/SysPulse.Host -f net8.0
dotnet new xunit -n SysPulse.Host.Tests -o tests/SysPulse.Host.Tests -f net8.0
dotnet sln add src/SysPulse.Host tests/SysPulse.Host.Tests
dotnet add tests/SysPulse.Host.Tests reference src/SysPulse.Host
dotnet add src/SysPulse.Host package LibreHardwareMonitorLib
```
...
`tests/SysPulse.Host.Tests/SensorSnapshotTests.cs`:
...
`src/SysPulse.Host/Model/SensorSnapshot.cs`:

```csharp
namespace SysPulse.Host.Model;
...
**Files:**
- Create: `src/SysPulse.Host/Sensors/ISensorReader.cs`
- Create: `src/SysPulse.Host/Sensors/SensorMapper.cs`
- Create: `src/SysPulse.Host/Sensors/LhmSensorReader.cs`
- Test: `tests/SysPulse.Host.Tests/SensorMapperTests.cs`

- [ ] **Step 1: 写失败测试（映射与过滤，纯函数，无硬件依赖）**

`tests/SysPulse.Host.Tests/SensorMapperTests.cs`:

```csharp
using LibreHardwareMonitor.Hardware;
using SysPulse.Host.Sensors;
using Xunit;
...
Run: `dotnet test tests/SysPulse.Host.Tests --filter SensorMapperTests`
Expected: FAIL —— `SensorMapper` 不存在。

- [ ] **Step 3: 实现 SensorMapper 与 ISensorReader**

`src/SysPulse.Host/Sensors/ISensorReader.cs`:
...
`src/SysPulse.Host/Sensors/SensorMapper.cs`:
...
`src/SysPulse.Host/Sensors/LhmSensorReader.cs`:
...
以**管理员**身份打开终端运行：
Run: `dotnet run --project src/SysPulse.Host -- --dump`
...
**Files:**
- Create: `src/SysPulse.Host/Ipc/PipeJsonServer.cs`
- Test: `tests/SysPulse.Host.Tests/PipeJsonServerTests.cs`

协议（v1，刻意最简）：客户端连接 `SysPulse.Host.v1` 管道 → 写一行 `GET` → 服务端回一行 JSON（`SensorSnapshot` 序列化）→ 断开。无版本协商、无订阅推送（YAGNI，轮询场景够用）。

- [ ] **Step 1: 写失败测试（进程内起服务端 + 客户端连通）**

`tests/SysPulse.Host.Tests/PipeJsonServerTests.cs`:
...
Run: `dotnet test tests/SysPulse.Host.Tests --filter PipeJsonServerTests`
...
**Files:**
- Create: `src/SysPulse.Host/app.manifest`
- Modify: `src/SysPulse.Host/Program.cs`（替换 Task 3 的临时版）
- Modify: `src/SysPulse.Host/SysPulse.Host.csproj`

- [ ] **Step 1: 提权清单**

`src/SysPulse.Host/app.manifest`:
...
const string PipeName = "SysPulse.Host.v1";
const int RefreshMs = 2000;

using var singleInstance = new Mutex(initiallyOwned: true, "Global\\SysPulse.Host", out var isNew);
if (!isNew)
{
    Console.Error.WriteLine("SysPulse.Host 已在运行，退出。");
    return 1;
}
...
Console.WriteLine($"SysPulse.Host 运行中，管道: {PipeName}，刷新间隔: {RefreshMs}ms。Ctrl+C 退出。");
...
```bash
dotnet build src/SysPulse.Host
```

资源管理器双击 `src/SysPulse.Host/bin/Debug/net8.0/SysPulse.Host.exe`（或管理员终端运行）→ 接受 UAC → 控制台显示运行中。再从**普通（非管理员）**终端验证跨完整性级别连通：

```powershell
$c = [System.IO.Pipes.NamedPipeClientStream]::new('.', 'SysPulse.Host.v1', 'InOut')
...
**Files:**
- Create: `src/SysPulseExtension/Dock/SensorDockBand.cs`
- Modify: `src/SysPulseExtension/SysPulseExtensionCommandsProvider.cs`

- [ ] **Step 1: 实现自刷新的 band item（先用假数据）**

`src/SysPulseExtension/Dock/SensorDockBand.cs`:
...
internal sealed partial class SensorDockBand : ListItem, IDisposable
{
    ...
    public SensorDockBand() : base(new NoOpCommand() { Id = "com.syspulse.dock.readings" })
    {
        Title = "CPU --°C";
...
`SysPulseExtensionCommandsProvider.cs` 中（替换 Task 1 Step 6 的临时实现）：
...
public override ICommandItem[]? GetDockBands()
{
    return [new WrappedDockItem([_band], "com.syspulse.dock", "SysPulse")];
}
```
...
**Files:**
- Create: `src/SysPulseExtension/Ipc/SensorSnapshot.cs`（Host 侧 DTO 的拷贝）
- Create: `src/SysPulseExtension/Ipc/PipeSensorClient.cs`
- Modify: `src/SysPulseExtension/Dock/SensorDockBand.cs`

- [ ] **Step 1: DTO 拷贝**

`src/SysPulseExtension/Ipc/SensorSnapshot.cs` —— 内容与 Host 的 `Model/SensorSnapshot.cs` 相同，仅命名空间改为 `SysPulseExtension.Ipc`。MVP 接受两份拷贝（协议 v1 字段稳定后再考虑共享项目；MSIX 项目引用普通 classlib 有打包坑，不值得现在处理）。

- [ ] **Step 2: 管道客户端**

`src/SysPulseExtension/Ipc/PipeSensorClient.cs`:
...
**Files:**
- Create: `src/SysPulseExtension/Commands/LaunchHostCommand.cs`
- Modify: `src/SysPulseExtension/Dock/SensorDockBand.cs`

- [ ] **Step 1: 拉起命令**

`src/SysPulseExtension/Commands/LaunchHostCommand.cs`:
...
关闭 Host → 设置环境变量 `SYSPULSE_HOST_EXE` 指向 Host 构建产物（**注意**：需在系统级设置或 CmdPal 进程能继承处设置，用户级 setx 后需重启 PowerToys）→ Reload 扩展。
