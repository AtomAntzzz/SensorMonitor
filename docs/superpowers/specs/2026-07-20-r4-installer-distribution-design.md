...
产出一个**签名的 Inno 安装器（.exe）**，让 SensorMonitor 能一键装到任意干净 Windows 机（无需装 .NET、无需 dev 环境变量、无需手动跑 `setup.ps1`）：装时一次 UAC 完成「装自包含 Host + 注册计划任务 + 注册扩展 MSIX」，装完运行期全程无 UAC，卸载一处清干净。消除 `SENSORMONITOR_HOST_EXE` 依赖。分发渠道 = **GitHub Release + WinGet**（installer type: inno），**不走 MS Store**。
...
- **Host 松散装 + 自包含**：Host 是纯 Win32 命名管道服务，不必进 MSIX。由安装器装到稳定路径 `%ProgramFiles%\SensorMonitor\Host\`；`dotnet publish --self-contained`（win-x64 / win-arm64）→ 目标机不依赖 .NET 8。
...
- 现有 Host 发现：`LaunchHostCommand.ResolveHostPath()` = env var `SENSORMONITOR_HOST_EXE`（dev）否则回退 `AppContext.BaseDirectory\Host\SensorMonitor.Host.exe`。
- 现有提权：计划任务 `SensorMonitor.Host`（`TaskInstaller`：`/Create /TN SensorMonitor.Host /TR "<ProcessPath>" /SC ONLOGON /RL HIGHEST /F`），`--install-task`/`--uninstall-task` 已存在；`schtasks /Run` 静默拉起、`/End` 停（均免提权）。`Environment.ProcessPath` 决定 `/TR`。
- MSIX 文件重定向：`%APPDATA%` 写被重定向进包私有存储；`%LOCALAPPDATA%` 行为含糊 → **跨包/进程稳定路径不能放 AppData**。`%ProgramFiles%` / `%ProgramData%` 不受此重定向影响（Host 今天就把日志写 `%ProgramData%\SensorMonitor\host.log`）。
...
- **Host 自包含发布**：`dotnet publish src/SensorMonitor.Host -c Release -r <rid> --self-contained -p:PublishSingleFile=false`，`<rid>` ∈ {`win-x64`, `win-arm64`}。产出含 .NET 运行时的 Host 目录。
- **扩展 MSIX**：复用 A2 链路构建 + 签名（`GenerateAppxPackageOnBuild` + signtool），**包内不加 Host**（与现状一致）。
- **Inno 打包**：新增 `installer/SensorMonitor.iss`（+ 构建脚本 `installer/build.ps1`）：把「对应架构的自包含 Host 目录」+「签名的扩展 .msix」+「签名证书公钥 .cer」收进安装器；编译出 `SensorMonitorSetup_<arch>.exe` 并用 A2 证书签名。x64 / ARM64 各一个安装器（或单安装器按 `ProcessorArchitecture` 选装对应 Host）。

### ② 安装器：安装流程（Inno，管理员一次 UAC）

`installer/SensorMonitor.iss` 关键段：
- `PrivilegesRequired=admin`（装 Host 到 ProgramFiles + 注册 HIGHEST 任务 + 预置 MSIX 均需提权）。
- `[Files]`：自包含 Host → `{autopf}\SensorMonitor\Host\`（= `%ProgramFiles%\SensorMonitor\Host\`）；扩展 `.msix`、`.cer` → `{tmp}`。
- `[Run]`（装完，提权上下文，均 `runhidden`）按序：
  1. **导入签名证书**（dev 证书才需；真证书跳过）：`certutil -addstore TrustedPeople "{tmp}\SensorMonitorDev.cer"` —— 否则预置签名 MSIX 失败。
  2. **注册计划任务**：跑**已装好的** `{autopf}\SensorMonitor\Host\SensorMonitor.Host.exe --install-task` —— `TaskInstaller.Install()` 以 `Environment.ProcessPath` 作 `/TR`，即 ProgramFiles 稳定路径。（等价直接 `schtasks /Create`，此处复用 Host 现成逻辑保持 DRY。）
  3. **注册扩展 MSIX（全机预置）**：`powershell -Command "Add-AppxProvisionedPackage -Online -PackagePath '{tmp}\SensorMonitorExtension_<ver>_<arch>.msix' -SkipLicense"`；并 `Add-AppxPackage -Register` 让**当前登录用户即时可用**（预置只保证新登录/其它用户；当前会话需显式注册一次）。
  4. **可选即时启动**：`schtasks /Run /TN SensorMonitor.Host`（装完立即有读数，免等下次登录）。
...
- **扩展唯一代码改动**：`LaunchHostCommand.ResolveHostPath()` 回退路径由 `AppContext.BaseDirectory\Host\...` 改为
  `Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.ProgramFiles), "SensorMonitor", "Host", "SensorMonitor.Host.exe")`。
...
- `[UninstallRun]`（卸载时，提权，`runhidden`）：
  1. `{app}\Host\SensorMonitor.Host.exe --uninstall-task`（删计划任务；等价 `schtasks /Delete /TN SensorMonitor.Host /F`）。先 `schtasks /End` 停掉在跑的 Host 免锁文件。
  2. `powershell -Command "Get-AppxPackage *SensorMonitorExtension* | Remove-AppxPackage; Remove-AppxProvisionedPackage -Online -PackageName <full>"` 移除扩展（当前用户 + 预置）。
- `[UninstallDelete]`：删 `{app}`（ProgramFiles\SensorMonitor）；删 `%ProgramData%\SensorMonitor\`（日志）。
- **双入口引导**：扩展 MSIX 的**包 DisplayName** 后缀区分（如 `Sensor Monitor (extension component)`），并在 README/Release note 注明"卸载请走『程序和功能 → Sensor Monitor』主入口，勿单独卸 MSIX 组件"（单独卸 MSIX 只清扩展、残留 Host+任务，退化为手动清理）。CmdPal 面板内展示名（`uap3:AppExtension DisplayName`）仍保持 `Sensor Monitor` 不受影响。
...
- **dev 流不受影响**：开发仍走 VS Deploy（松散 MSIX）+ `SENSORMONITOR_HOST_EXE` + `setup.ps1`；安装器只服务分发。

## 测试

- **Host 单测**：Host 近乎零改 → 现有 12 单测应仍全绿（回归确认，非新增）。
- **手动验收（关键，需干净 VM 或第二台机）**：
  1. 跑 `SensorMonitorSetup_x64.exe` → **一次 UAC** → 无报错完成。
  2. 打开 CmdPal → 出现 Sensor Monitor 扩展 + Dock 有实时读数（**无 `SENSORMONITOR_HOST_EXE`、无 .NET 8 预装**）。
  3. 重启机器 → 登录后任务自启、读数恢复，全程无 UAC。
  4. 卸载（走 Inno 主入口）→ 计划任务、`%ProgramFiles%\SensorMonitor\`、`%ProgramData%\SensorMonitor\`、扩展包**全部清除**。
  5. 覆盖装高版本 → 正常升级、读数正常。

## 验收清单

1. `installer/` 产出**签名**的 `SensorMonitorSetup_x64.exe`（ARM64 同理），内含自包含 Host + 签名 MSIX + 证书。
