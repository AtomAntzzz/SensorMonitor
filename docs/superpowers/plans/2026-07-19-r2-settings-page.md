...
> **测试策略（重要，源自已批准 spec）**：本扩展**无单测工程**（仅 `tests/SysPulse.Host.Tests`，net8+Host），
...
> 每次 build 前执行 `taskkill //f //im SysPulseExtension.exe 2>/dev/null || true`（无需提权）。
...
**Files:**
- Create: `src/SysPulseExtension/SysPulseExtension/Settings/TempDisplay.cs`
...
```bash
git add src/SysPulseExtension/SysPulseExtension/Settings/TempDisplay.cs
...
**Files:**
- Modify: `src/SysPulseExtension/SysPulseExtension/Ipc/SnapshotCache.cs`
...
```bash
git add src/SysPulseExtension/SysPulseExtension/Ipc/SnapshotCache.cs
...
> `JsonSettingsManager`：设 `FilePath = Utilities.BaseSettingsPath("SysPulseExtension")/settings.json`，
...
**Files:**
- Create: `src/SysPulseExtension/SysPulseExtension/Settings/SettingsManager.cs`

- [ ] **Step 1: 创建 SettingsManager**

两个 `ChoiceSetSetting`；`SettingsChanged` 与构造末尾各调一次 `Apply()`（首帧前推持久化初值）。
注意 `using` 别名避开命名空间 `SysPulseExtension.Settings` 与 Toolkit 类型 `Settings` 撞名：
...
```bash
git add src/SysPulseExtension/SysPulseExtension/Settings/SettingsManager.cs
...
在构造函数 `SysPulseExtensionCommandsProvider()` 里（第 24 行 `Icon = ...` 之后、`_commands = ...` 之前）加：
...
```bash
git add src/SysPulseExtension/SysPulseExtension/SysPulseExtensionCommandsProvider.cs
...
**Files:**
- Modify: `src/SysPulseExtension/SysPulseExtension/Dock/SensorSlotBand.cs:99`
- Modify: `src/SysPulseExtension/SysPulseExtension/Pages/SensorPickerPage.cs:55`
- Modify: `src/SysPulseExtension/SysPulseExtension/Pages/SysPulseExtensionPage.cs:46`
...
- [ ] **Step 3: 浏览页（SysPulseExtensionPage.GetItems）**
...
```bash
git add src/SysPulseExtension/SysPulseExtension/Dock/SensorSlotBand.cs \
        src/SysPulseExtension/SysPulseExtension/Pages/SensorPickerPage.cs \
        src/SysPulseExtension/SysPulseExtension/Pages/SysPulseExtensionPage.cs
...
VS 打开 `src/SysPulseExtension/SysPulseExtension.sln` → 右键扩展项目 **Deploy**（x64）→ CmdPal 内 Reload。

- [ ] **Step 2: 验收——设置页出现**

CmdPal 设置里出现 **SysPulse** 扩展的两项：刷新间隔、温度单位。
