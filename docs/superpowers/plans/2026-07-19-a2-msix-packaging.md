...
- 命令均在仓库根 `D:/Workspace/SysPulse` 执行；项目目录 = `src/SysPulseExtension/SysPulseExtension/`。
- manifest = `src/SysPulseExtension/SysPulseExtension/Package.appxmanifest`：当前 `Name="SysPulseExtension"`、`Publisher="CN=Microsoft Corporation, O=Microsoft Corporation, L=Redmond, S=Washington, C=US"`、`Version="0.0.1.0"`。
- csproj = `src/SysPulseExtension/SysPulseExtension/SysPulseExtension.csproj`：已有 `EnableMsixTooling=true`、Release 段 `PublishTrimmed=true`、`ILLinkTreatWarningsAsErrors=false`。
- 松散注册 AppxManifest：`src/SysPulseExtension/SysPulseExtension/bin/x64/Debug/net9.0-windows10.0.26100.0/win-x64/AppxManifest.xml`（Debug 构建时生成，随身份变更同步更新）。
- 当前装的包 `SysPulseExtension_0.0.1.0_x64__8wekyb3d8bbwe`（PublisherId 由 CN 派生，改身份后必变 → 包家族名变 → `slots.json` 重置一次，已接受）。
...
| `.../SysPulseExtension.csproj` | 修改 | 补 AppxPackage 身份三属性 |
| `.gitignore` | 修改 | 忽略 `AppPackages/`、`*.cer`、`*.pfx` |
| `docs/references/msix-packaging.md` | 新增 | 可复现打包命令序列 + 裁剪发现 |
| `CLAUDE.md` | 修改 | 文档地图挂指针 + 状态行 |

> `AppPackages/`（`.msix`/`.msixbundle`）、证书文件均为 gitignore 产物，不入库。下文路径省略前缀 `src/SysPulseExtension/SysPulseExtension/`（记作 `<PROJ>/`）。
...
- Modify: `<PROJ>/SysPulseExtension.csproj`
- Modify: `.gitignore`
...
```xml
  <Identity
    Name="SysPulseExtension"
    Publisher="CN=SysPulse Dev"
    Version="0.0.1.0" />
```

- [ ] **Step 2: csproj 补身份三属性**

`SysPulseExtension.csproj` 首个 `<PropertyGroup>`（含 `<OutputType>WinExe</OutputType>`）里、`</PropertyGroup>` 之前加：

```xml
    <AppxPackageIdentityName>SysPulseExtension</AppxPackageIdentityName>
    <AppxPackagePublisher>CN=SysPulse Dev</AppxPackagePublisher>
...
```bash
powershell -NoProfile -Command "Get-AppxPackage *SysPulse* | Remove-AppxPackage; Add-AppxPackage -Register 'D:\Workspace\SysPulse\src\SysPulseExtension\SysPulseExtension\bin\x64\Debug\net9.0-windows10.0.26100.0\win-x64\AppxManifest.xml'; (Get-AppxPackage *SysPulse*).PackageFullName"
```
...
```bash
cd "D:/Workspace/SysPulse" && git add src/SysPulseExtension/SysPulseExtension/Package.appxmanifest src/SysPulseExtension/SysPulseExtension/SysPulseExtension.csproj .gitignore && git commit -m "chore(ext): adopt CN=SysPulse Dev identity, ignore MSIX artifacts"
```
...
Export-Certificate -Cert $cert -FilePath "$env:TEMP\SysPulseDev.cer" | Out-Null
"CER=$env:TEMP\SysPulseDev.cer"
EOF
powershell -NoProfile -ExecutionPolicy Bypass -File /tmp/mkcert.ps1
```
Expected: 打印 `THUMBPRINT=<40位十六进制>` 与 `CER=...SysPulseDev.cer`。**记下 THUMBPRINT**，Task 3/5 签名要用。

- [ ] **Step 2: 信任导入（提权，一次 UAC）**

```bash
powershell -NoProfile -Command "Start-Process powershell -Verb RunAs -ArgumentList '-NoProfile','-Command','Import-Certificate -FilePath \"$env:TEMP\SysPulseDev.cer\" -CertStoreLocation Cert:\LocalMachine\TrustedPeople'"
```
...
```bash
powershell -NoProfile -Command "Get-ChildItem Cert:\LocalMachine\TrustedPeople | Where-Object Subject -eq 'CN=SysPulse Dev' | Select-Object Subject, Thumbprint | Format-List; Get-ChildItem Cert:\CurrentUser\My | Where-Object Subject -eq 'CN=SysPulse Dev' | Select-Object HasPrivateKey | Format-List"
```
Expected: TrustedPeople 里见 `CN=SysPulse Dev` + thumbprint；CurrentUser\My 里 `HasPrivateKey : True`（签名需私钥）。两处 thumbprint 应与 Step 1 一致。
...
```bash
cd "D:/Workspace/SysPulse" && taskkill //f //im SysPulseExtension.exe 2>/dev/null; dotnet build src/SysPulseExtension/SysPulseExtension/SysPulseExtension.csproj -c Release -p:Platform=x64 -p:GenerateAppxPackageOnBuild=true -p:AppxBundle=Never -p:AppxPackageDir="AppPackages\x64\\" 2>&1 | tail -5
```
...
```bash
ls -R src/SysPulseExtension/SysPulseExtension/AppPackages/x64/ 2>/dev/null | grep -i "\.msix$"
```
Expected: 见形如 `SysPulseExtension_0.0.1.0_x64.msix` 的文件。记其完整路径为 `<X64MSIX>`。
...
```bash
cd "D:/Workspace/SysPulse" && dotnet build src/SysPulseExtension/SysPulseExtension/SysPulseExtension.csproj -c Release -p:Platform=ARM64 -p:GenerateAppxPackageOnBuild=true -p:AppxBundle=Never -p:AppxPackageDir="AppPackages\ARM64\\" 2>&1 | tail -5
```
...
```bash
cd "D:/Workspace/SysPulse/src/SysPulseExtension/SysPulseExtension" && mkdir -p AppPackages/bundle_stage && cp AppPackages/x64/*.msix AppPackages/ARM64/*.msix AppPackages/bundle_stage/ && ls AppPackages/bundle_stage/
```
...
```bash
cd "D:/Workspace/SysPulse/src/SysPulseExtension/SysPulseExtension" && "/c/Program Files (x86)/Windows Kits/10/bin/10.0.26100.0/x64/makeappx.exe" bundle //d AppPackages/bundle_stage //p AppPackages/SysPulseExtension_0.0.1.0.msixbundle
```
Expected: `Package creation succeeded`，生成 `AppPackages/SysPulseExtension_0.0.1.0.msixbundle`。

- [ ] **Step 3: 签名 bundle**

```bash
cd "D:/Workspace/SysPulse/src/SysPulseExtension/SysPulseExtension" && "/c/Program Files (x86)/Windows Kits/10/bin/10.0.26100.0/x64/signtool.exe" sign //fd SHA256 //sha1 <THUMB> "AppPackages/SysPulseExtension_0.0.1.0.msixbundle"
```
...
```bash
powershell -NoProfile -Command "Get-AppxPackage *SysPulse* | Remove-AppxPackage; Add-AppxPackage -Path '<X64MSIX>'; (Get-AppxPackage *SysPulse*) | Select-Object PackageFullName, SignatureKind | Format-List"
```
...
```bash
powershell -NoProfile -Command "schtasks /Run /TN SysPulse.Host 2>&1 | Out-Null; Start-Sleep 3; (Get-Process SysPulse.Host -ErrorAction SilentlyContinue) | Select-Object Id"
```
...
```bash
cd "D:/Workspace/SysPulse" && taskkill //f //im SysPulseExtension.exe 2>/dev/null; dotnet build src/SysPulseExtension/SysPulseExtension/SysPulseExtension.csproj -c Debug -p:Platform=x64 2>&1 | tail -2
```
...
> A2 验证产出（2026-07-19）。身份 = `CN=SysPulse Dev`（自签名 dev，商店提交时换 Partner Center）。
> 工具：Windows Kits 10.0.26100.0 的 signtool/makeappx（`C:\Program Files (x86)\Windows Kits\10\bin\10.0.26100.0\x64\`）。

## 一次性准备
- 证书：`New-SelfSignedCertificate -Type CodeSigningCert -Subject "CN=SysPulse Dev" -CertStoreLocation Cert:\CurrentUser\My -KeyExportPolicy Exportable`
...
## 本机实装
    Get-AppxPackage *SysPulse* | Remove-AppxPackage
    Add-AppxPackage -Path AppPackages\x64\*.msix
（Host 不在包内 —— 靠计划任务 SysPulse.Host 拉起，R4 才随包分发。）
...
```bash
cd "D:/Workspace/SysPulse" && git add docs/references/msix-packaging.md CLAUDE.md docs/plans/2026-07-18-verification-and-next-phase.md && git commit -m "docs: A2 MSIX packaging validated, add repro reference" && git push
```
...
- **一致性**：身份串 `CN=SysPulse Dev` 跨 manifest/csproj/证书/签名统一；signtool/makeappx 全路径统一 `10.0.26100.0/x64`；`AppPackages/` 目录结构（x64//ARM64//bundle_stage）跨 Task 3/4/5/6 一致；松散注册 AppxManifest 路径跨 Task 1/7 一致。
