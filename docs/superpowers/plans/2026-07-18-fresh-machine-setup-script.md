...
**Goal:** 交付一个自提权、幂等的 `scripts/setup.ps1`（+ `setup.cmd` 包装），把 SysPulse 在全新 Windows 机器上从零拉起到"能构建、11 单测绿、扩展已尝试部署、Host 计划任务已注册"。
...
- 仓库根 = `SysPulse.sln` 所在目录；`scripts/` 将建在其下，脚本用 `$PSScriptRoot\..` 上溯定位根。
- Host exe 构建输出：`src/SysPulse.Host/bin/Debug/net8.0/SysPulse.Host.exe`（`dotnet build` Debug 默认路径）。
- Host 已有 `--install-task`（注册计划任务 `SysPulse.Host`）与 `--dump` 参数（见 `src/SysPulse.Host/Program.cs`）。
- Host 单测项目：`tests/SysPulse.Host.Tests`，当前 11 测全绿。
- 扩展项目：`src/SysPulseExtension/SysPulseExtension/SysPulseExtension.csproj`，`TargetFramework=net9.0-windows10.0.26100.0`，`RuntimeIdentifiers=win-x64;win-arm64`，`EnableMsixTooling=true`。
...
Run（Git Bash）：
```bash
cd "D:/Workspace/SysPulse" && powershell -ExecutionPolicy Bypass -NoProfile -File scripts/setup.ps1 -CheckOnly 2>&1 | head -20
```
Expected: 打印 `==> SysPulse 引导开始（RepoRoot=...SysPulse...）` 和 `===== 总结 =====` 空表，无报错、退出码 0。

- [ ] **Step 3: Commit**

```bash
cd "D:/Workspace/SysPulse" && git add scripts/setup.ps1 && git commit -m "feat(setup): script skeleton with summary collector"
```
...
```bash
cd "D:/Workspace/SysPulse" && git add scripts/setup.ps1 && git commit -m "feat(setup): stage 0 preflight and self-elevation"
```
...
Run:
```bash
cd "D:/Workspace/SysPulse" && powershell -ExecutionPolicy Bypass -NoProfile -File scripts/setup.ps1 -CheckOnly 2>&1 | tail -25
```
...
```bash
cd "D:/Workspace/SysPulse" && git add scripts/setup.ps1 && git commit -m "feat(setup): stage 1 idempotent toolchain install via winget"
```
...
Run:
```bash
cd "D:/Workspace/SysPulse" && powershell -ExecutionPolicy Bypass -NoProfile -File scripts/setup.ps1 -CheckOnly 2>&1 | tail -15
```
...
```bash
cd "D:/Workspace/SysPulse" && git add scripts/setup.ps1 && git commit -m "feat(setup): stage 2 enable developer mode"
```
...
Run:
```bash
cd "D:/Workspace/SysPulse" && powershell -ExecutionPolicy Bypass -NoProfile -File scripts/setup.ps1 -CheckOnly 2>&1 | tail -12
```
...
```bash
cd "D:/Workspace/SysPulse" && dotnet test tests/SysPulse.Host.Tests -c Debug 2>&1 | tail -3
```
...
```bash
cd "D:/Workspace/SysPulse" && git add scripts/setup.ps1 && git commit -m "feat(setup): stage 3 host build and test"
```
...
```bash
cd "D:/Workspace/SysPulse" && powershell -ExecutionPolicy Bypass -NoProfile -Command "& { . { $null } ; Add-AppxPackage -Register 'src/SysPulseExtension/SysPulseExtension/bin/x64/Debug/net9.0-windows10.0.26100.0/win-x64/AppxManifest.xml' ; Get-AppxPackage *SysPulse* | Select-Object Name,Status }" 2>&1 | tail -3
```
Expected: `SysPulseExtension ... Ok`（spike 已确认）。

- [ ] **Step 5: Commit**

```bash
cd "D:/Workspace/SysPulse" && git add scripts/setup.ps1 && git commit -m "feat(setup): stage 4 CLI extension deploy with auto-reload"
```
...
Run:
```bash
cd "D:/Workspace/SysPulse" && powershell -ExecutionPolicy Bypass -NoProfile -File scripts/setup.ps1 -CheckOnly 2>&1
```
...
```bash
cd "D:/Workspace/SysPulse" && git add scripts/setup.ps1 && git commit -m "feat(setup): stage 5 scheduled task and full pipeline wiring"
```
...
Run:
```bash
cd "D:/Workspace/SysPulse" && cmd //c "scripts\\setup.cmd -CheckOnly" 2>&1 | tail -12
```
...
```bash
cd "D:/Workspace/SysPulse" && git add scripts/setup.cmd CLAUDE.md && git commit -m "feat(setup): cmd wrapper and CLAUDE.md command reference"
```
...
| P12 | 阶段 4 扩展构建在真机实跑失败（MSB3027/MSB3021：`SysPulseExtension.exe` 被自身进程锁定）：已注册的松散包被 CmdPal 激活后扩展进程常驻，锁死自己的 bin 输出——**坑 #6 的扩展版**。spike 当时能过是因为扩展进程尚未被拉起 | Task 6 构建前先 `taskkill SysPulseExtension.exe`（CmdPal 按需重拉，末尾 reload 亦会重启）；修复序列已真机验证（杀→构建 0 错→注册 Ok→reload） | 真机阶段 4 失败复现 + 修复后实跑 |
...
- 阶段 3 整 sln 构建**无平台映射问题**：`SysPulse.sln` 只含 Host + Tests（均 AnyCPU；扩展项目在独立 sln，阶段 4 单独带 `-p:Platform=x64` 构建），且 `dotnet build SysPulse.sln -c Debug` 已在本机实测通过（0 错误）。
