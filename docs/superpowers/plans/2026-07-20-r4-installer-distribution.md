...
**Architecture:** 扩展仍是完整签名 MSIX（CmdPal 已验证发现路径，且 Release 裁剪=自包含，带自己的 .NET 运行时）。Host 改自包含发布、由安装器装到 `%ProgramFiles%\SensorMonitor\Host\`。Inno 安装器（提权）装 Host + 跑 `Host.exe --install-task` 注册 `/RL HIGHEST` 任务 + `Add-AppxProvisionedPackage` 全机预置扩展；卸载反向清干净。扩展仅改 `ResolveHostPath` 一处回退路径。
...
**Files:**
- Modify: `src/SensorMonitorExtension/SensorMonitorExtension/Commands/LaunchHostCommand.cs:19-25`
...
```csharp
    ...
    /// %ProgramFiles%\SensorMonitor\Host\ —— 安装器装 Host 到此稳定路径、计划任务 /TR 亦指向它。
    /// 例：SENSORMONITOR_HOST_EXE=D:\Workspace\SensorMonitor\src\SensorMonitor.Host\bin\Debug\net8.0\SensorMonitor.Host.exe
    /// </summary>
    internal static string ResolveHostPath() =>
        Environment.GetEnvironmentVariable("SENSORMONITOR_HOST_EXE")
        ?? Path.Combine(
            ...
            "SensorMonitor", "Host", "SensorMonitor.Host.exe");
```
...
```bash
git add src/SensorMonitorExtension/SensorMonitorExtension/Commands/LaunchHostCommand.cs
git commit -m "feat: Host 回退路径改指 %ProgramFiles%\\SensorMonitor\\Host（R4 安装器就位）"
```

---

### Task 2: Inno 安装器脚本 SensorMonitor.iss

**Files:**
- Create: `installer/SensorMonitor.iss`
...
```bash
git add installer/SensorMonitor.iss
...
```bash
taskkill //f //im SensorMonitor.Host.exe 2>/dev/null || true
powershell.exe -NoProfile -ExecutionPolicy Bypass -File installer/build.ps1 -Arch x64 2>&1 | tail -20
```

Expected: 结尾 `OK → ...\installer\Output\SensorMonitorSetup_x64.exe`，无 throw。若 ISCC 报 `[Run]` 引号/转义错 → 回 Task 2 微调 PowerShell 引号后重跑。

- [ ] **Step 2: 验证产出物 + 签名**

```bash
ls -la installer/Output/SensorMonitorSetup_x64.exe
"/c/Program Files (x86)/Windows Kits/10/bin/10.0.26100.0/x64/signtool.exe" verify /pa installer/Output/SensorMonitorSetup_x64.exe 2>&1 | tail -3
```
...
无源码改动则跳过提交；若 Step 1 回改了 `SensorMonitor.iss`：
...
拷 `installer/Output/SensorMonitorSetup_x64.exe` 到干净机，双击安装（**一次 UAC**）。逐条验：
1. 安装无报错完成。
2. 打开 CmdPal → 出现 Sensor Monitor 扩展 + Dock 有实时读数（**未设 `SENSORMONITOR_HOST_EXE`、未预装 .NET**）。
3. 任务计划程序里有 `SensorMonitor.Host`（`/TR` = `%ProgramFiles%\SensorMonitor\Host\SensorMonitor.Host.exe`）。
4. 重启机器 → 登录后读数自动恢复，全程无 UAC。

- [ ] **Step 3: 卸载验收**

「程序和功能 → Sensor Monitor」卸载 → 验：计划任务消失、`%ProgramFiles%\SensorMonitor\` 与 `%ProgramData%\SensorMonitor\` 删除、CmdPal 里扩展消失（`Get-AppxPackage *SensorMonitor*` 空）。
...
- **类型/签名一致**：`ResolveHostPath` 回退 = `%ProgramFiles%\SensorMonitor\Host\SensorMonitor.Host.exe`（Task 1 定义）与安装器 `[Files]` DestDir `{app}\Host`（`{app}`=`{autopf}\SensorMonitor`）、任务 `/TR`、Task 6 验收路径三处一致 ✓；`build.ps1` 传入的 `/D` 变量名（MyArch/MyVersion/MyHostDir/MyMsix/MyMsixName/MyCer/MyCerName）与 `.iss` 引用一致 ✓；证书名 `SensorMonitorDev.cer` 三处一致 ✓；任务名 `SensorMonitor.Host` 全篇一致 ✓。
